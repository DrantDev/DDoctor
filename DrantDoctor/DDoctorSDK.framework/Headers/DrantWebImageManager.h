...
typedef NS_OPTIONS(NSUInteger, DrantWebImageOptions) {
    /**
     * By default, when a URL fail to be downloaded, the URL is blacklisted so the library won't keep trying.
     * This flag disable this blacklisting.
     */
    DrantWebImageRetryFailed = 1 << 0,

    /**
     * By default, image downloads are started during UI interactions, this flags disable this feature,
     * leading to delayed download on UIScrollView deceleration for instance.
     */
    DrantWebImageLowPriority = 1 << 1,

    /**
     * This flag disables on-disk caching after the download finished, only cache in memory
     */
    DrantWebImageCacheMemoryOnly = 1 << 2,

    /**
     * This flag enables progressive download, the image is displayed progressively during download as a browser would do.
     * By default, the image is only displayed once completely downloaded.
     */
    DrantWebImageProgressiveDownload = 1 << 3,

    /**
     * Even if the image is cached, respect the HTTP response cache control, and refresh the image from remote location if needed.
     * The disk caching will be handled by NSURLCache instead of DrantWebImage leading to slight performance degradation.
     * This option helps deal with images changing behind the same request URL, e.g. Facebook graph api profile pics.
     * If a cached image is refreshed, the completion block is called once with the cached image and again with the final image.
     *
     * Use this flag only if you can't make your URLs static with embedded cache busting parameter.
     */
    DrantWebImageRefreshCached = 1 << 4,

    /**
     * In iOS 4+, continue the download of the image if the app goes to background. This is achieved by asking the system for
     * extra time in background to let the request finish. If the background task expires the operation will be cancelled.
     */
    DrantWebImageContinueInBackground = 1 << 5,

    /**
     * Handles cookies stored in NSHTTPCookieStore by setting
     * NSMutableURLRequest.HTTPShouldHandleCookies = YES;
     */
    DrantWebImageHandleCookies = 1 << 6,

    /**
     * Enable to allow untrusted SSL certificates.
     * Useful for testing purposes. Use with caution in production.
     */
    DrantWebImageAllowInvalidSSLCertificates = 1 << 7,

    /**
     * By default, images are loaded in the order in which they were queued. This flag moves them to
     * the front of the queue.
     */
    DrantWebImageHighPriority = 1 << 8,
    
    /**
     * By default, placeholder images are loaded while the image is loading. This flag will delay the loading
     * of the placeholder image until after the image has finished loading.
     */
    DrantWebImageDelayPlaceholder = 1 << 9,
     ...
     */
    DrantWebImageTransformAnimatedImage = 1 << 10,
     ...
     */
    DrantWebImageAvoidAutoSetImage = 1 << 11,
    
    /**
     * By default, images are decoded respecting their original size. On iOS, this flag will scale down the
     * images to a size compatible with the constrained memory of devices.
     * If `DrantWebImageProgressiveDownload` flag is set the scale down is deactivated.
     */
    DrantWebImageScaleDownLargeImages = 1 << 12,
    
    /**
     * By default, we do not query disk data when the image is cached in memory. This mask can force to query disk data at the same time.
     * This flag is recommend to be used with `DrantWebImageQueryDiskSync` to ensure the image is loaded in the same runloop.
     */
    DrantWebImageQueryDataWhenInMemory = 1 << 13,
     ...
     */
    DrantWebImageQueryDiskSync = 1 << 14,
     ...
     */
    DrantWebImageFromCacheOnly = 1 << 15,
    /**
     * By default, when you use `DrantWebImageTransition` to do some view transition after the image load finished, this transition is only applied for image download from the network. This mask can force to apply view transition for memory and disk cache as well.
     */
...
typedef NSData * _Nullable(^DrantWebImageCacheSerializerBlock)(UIImage * _Nonnull image, NSData * _Nullable data, NSURL * _Nullable imageURL);


@class DrantWebImageManager;

@protocol DrantWebImageManagerDelegate <NSObject>
 ...
 *
 * @param imageManager The current `DrantWebImageManager`
 * @param imageURL     The url of the image to be downloaded
 *
 * @return Return NO to prevent the downloading of the image on cache misses. If not implemented, YES is implied.
 */
- (BOOL)imageManager:(nonnull DrantWebImageManager *)imageManager shouldDownloadImageForURL:(nullable NSURL *)imageURL;
 ...
 */
- (BOOL)imageManager:(nonnull DrantWebImageManager *)imageManager shouldBlockFailedURL:(nonnull NSURL *)imageURL withError:(nonnull NSError *)error;
 ...
 */
- (nullable UIImage *)imageManager:(nonnull DrantWebImageManager *)imageManager transformDownloadedImage:(nullable UIImage *)image withURL:(nullable NSURL *)imageURL;
 ...
 * a UIView.
 *
 * Here is a simple example of how to use DrantWebImageManager:
 *
 * @code

DrantWebImageManager *manager = [DrantWebImageManager sharedManager];
 ...
 * @endcode
 */
@interface DrantWebImageManager : NSObject

@property (weak, nonatomic, nullable) id <DrantWebImageManagerDelegate> delegate;

@property (strong, nonatomic, readonly, nullable) DrantImageCache *imageCache;
@property (strong, nonatomic, readonly, nullable) DrantWebImageDownloader *imageDownloader;

/**
 * The cache filter is a block used each time DrantWebImageManager need to convert an URL into a cache key. This can
...
DrantWebImageManager.sharedManager.cacheKeyFilter = ^(NSURL * _Nullable url) {
 ...
 */
@property (nonatomic, copy, nullable) DrantWebImageCacheSerializerBlock cacheSerializer;

/**
 * Returns global DrantWebImageManager instance.
 *
 * @return DrantWebImageManager shared instance
 */
+ (nonnull instancetype)sharedManager;

/**
 * Allows to specify instance of cache and image downloader used with image manager.
 * @return new instance of `DrantWebImageManager` with specified cache and downloader.
 */
- (nonnull instancetype)initWithCache:(nonnull DrantImageCache *)cache downloader:(nonnull DrantWebImageDownloader *)downloader NS_DESIGNATED_INITIALIZER;
...
- (void)cachedImageExistsForURL:(nullable NSURL *)url
                     completion:(nullable DrantWebImageCheckCacheCompletionBlock)completionBlock;
